<template>
  <div class="userList">
    <div class="wrapper">
        ...
        <el-table :data="listData" border style="width: 100%">
          ...
          <el-table-column label='操作' width='200'>
            <template slot-scope="scope">
              ...
              <el-button @click="deleteKitchenLicense(scope.row)" type="text" size="small">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
      ...
      <div class="dialogBox">
        <el-dialog title="详情" :visible.sync="dialogViewVisible" :show-close="true" width="1100px">
          <div class="wrapper">
            <el-form :model="form" :rules="rules" label-width="120px" ref="ruleForm">
              <el-row>
                <el-col :span="24">
                  ...
                  <el-form-item>
                    <img style="width:90%;" :src="viewImage.url" />
                  </el-form-item>
                </el-col>
              </el-row>
            </el-form>
        ...
        <el-dialog title="新增文件" :visible.sync="dialogVisible" :show-close="true" width="750px">
          <div class="wrapper">
            <el-form :model="form" :rules="rules" label-width="120px" ref="ruleForm">
              <el-row>
                <el-col :span="24">
                  <el-form-item style="width:300px;" label="供应商：" prop="kitchenId">
                    <el-select v-model.trim="form.kitchenId" placeholder="请选择供应商" size="small">
...
export default {
  ...
  methods: {
    getKitchenLicenseList (pageIndex = 1, pageSize = 15) {
      let datas = {
      ...
      OrderModule.getKitchenLicenseList(datas).then(res => {
        let list = [];
        let total = 0;
    ...
    // 查询
    submitForm () {
      this.getKitchenLicenseList();
    },
    // 分页
    handleSizeChange (val) {
      this.pageSize = val;
      this.getKitchenLicenseList(this.pageIndex, val);
    },
    handleCurrentChange (val) {
      this.pageIndex = val;
      this.getKitchenLicenseList(val, this.pageSize);
    },
    download (codeIMG) {
    ...
    importDiner () {
      ...
      OrderModule.addKitchenLicenseInfo(datas).then(res => {
        if (res.data) {
          if (res.data.err == 0) {
            this.$message({
              type: 'success',
    ...
    deleteKitchenLicense (row) {
      this.$confirm(`是否删除文件?`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.deleteKitchenLicenseInfo(row.id);
        // 成功后回掉
      }).catch(() => {
        // this.$message({
        //     type: 'info',
    ...
    deleteKitchenLicenseInfo (id) {
      OrderModule.deleteKitchenLicenseInfo(id).then(res => {
        if (res.data) {
          if (res.data.err == 0) {
            this.$message({
              type: 'success',
