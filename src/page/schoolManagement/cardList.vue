            ...
            <el-col :span="18">
              ...
              <el-form-item prop="cardNum" style="width: 200px">
                <el-input
                  size="small"
                  clearable
                  v-model.trim="formInline.cardNum"
                  placeholder="请输入餐卡编号"
                ></el-input>
              </el-form-item>
              <el-form-item prop="gradeId">
                <el-select
                  clearable
                  filterable
                  v-model.trim="formInline.gradeId"
                  placeholder="请选择部门"
                  size="small"
                >
                  <el-option
              ...
              <el-form-item prop="cardStatus" style="width: 180px">
                <el-select
                  clearable
                  v-model.trim="formInline.cardStatus"
                  placeholder="是否绑定状态"
                  size="small"
                >
                  <el-option
        ...
        <el-table :data="listData" border style="width: 100%">
          ...
          >
            <template slot-scope="scope">
              <el-button @click="updateData(scope.row)" type="text" size="small"
                >修改</el-button
              >
              <el-button @click="changeData(scope.row)" type="text" size="small"
                >解绑</el-button
              >
            </template>
          </el-table-column>
...
export default {
  data() {
    return {
      ...
      rules: {
        gradeId: [{ required: true, message: "请选择", trigger: "change" }],
        dinerId: [{ required: true, message: "请选择", trigger: "change" }],
      },
    };
  },
  mounted() {
      ...
      CardModule.getGinerList(datas).then((res) => {
        if (res.data) {
          ...
          res.data.list.forEach((el, i) => {
            list.push({
              id: el.id,
              title: el.card_number
                ? el.username + " --- " + el.card_number
                : el.username,
